...
#     @staticmethod
#     def resize_to_divisible_by_14(x):
#         """
#         Resizes the input tensor so that its height and width are divisible by 14.
#         """


#         # Calculate new dimensions divisible by 14
#         new_height = (x.shape[2] // 14) * 14
#         new_width = (x.shape[3] // 14) * 14
...
#     def forward(self, x):
#         x = self.resize_to_divisible_by_14(x)
...
class DinoV2(nn.Module):
    def __init__(self, shape=(640, 640), out_feature_indexes=[1, 2, 3], freeze=False):
        super().__init__()

        # Load ConvNeXtV2 from timm with intermediate features
        self.backbone = timm.create_model(
            'convnextv2_base',
            pretrained=True,
            features_only=True  # important: get intermediate stages
        )
        ...
        # Save the output feature channels for selected stages
        feature_info = self.backbone.feature_info
        self.out_feature_indexes = out_feature_indexes
        self._out_feature_channels = [feature_info[i]['num_chs'] for i in out_feature_indexes]
        print(f"[ConvNeXtV2] Selected output indexes: {self.out_feature_indexes}")
        print(f"[ConvNeXtV2] Output feature channels: {self._out_feature_channels}")

        self.shape = shape

    def forward(self, x):
        x = self.resize_to_divisible_by_32(x)
        features = self.backbone(x)  # list of feature maps from all stages
        outputs = [features[i] for i in self.out_feature_indexes]
        return outputs

    @staticmethod
    def resize_to_divisible_by_32(x):
        new_height = (x.shape[2] // 32) * 32
        new_width = (x.shape[3] // 32) * 32
        return F.interpolate(x, size=(new_height, new_width), mode="bilinear", align_corners=False)
