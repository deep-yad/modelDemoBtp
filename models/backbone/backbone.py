...
class Backbone(BackboneBase):
    ...
    def __init__(self,
        ...
        self.encoder = DinoV2(
                shape=target_shape,
                out_feature_indexes=[1, 2, 3],  # ConvNeXt: last 3 stages
                freeze=freeze_encoder,
        )
        #print(f"[ConvNeXt] Encoder output channels before setting manually: {self.encoder._out_feature_channels}")
        self.encoder._out_feature_channels = [256, 512, 1024]
        #print(f"[ConvNeXt] Encoder output channels: {self.encoder._out_feature_channels}")
    ...
    def forward(self, tensor_list: NestedTensor):
        ...
        #print("[DEBUG] Encoder type:", type(self.encoder))
        #print("[DEBUG] Encoder modules:\n", self.encoder)

        # Get raw ConvNeXt feature maps directly (list of 3 tensors)
        feats = self.encoder(tensor_list.tensors)  # output: [B, C_i, H_i, W_i] for each i

        # DEBUG PRINT
        #for i, f in enumerate(feats):
        #    print(f"[Encoder Output] Feature {i} shape: {f.shape}")
        
        feats = self.projector(feats)

        # DEBUG PRINT
        #for i, f in enumerate(feats):
        #    print(f"[Projector Output] Feature {i} shape: {f.shape}")
