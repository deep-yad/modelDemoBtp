...
class MSDeformAttn(nn.Module):
    ...
    def __init__(self, d_model=256, n_levels=3, n_heads=8, n_points=4):
        ...
        self.sampling_offsets = nn.Linear(d_model, n_heads * 3 * n_points * 2)
        self.attention_weights = nn.Linear(d_model, n_heads * 3 * n_points)
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes,
        ...
        sampling_offsets = self.sampling_offsets(query).view(N, Len_q, self.n_heads, 3, self.n_points, 2)
        attention_weights = self.attention_weights(query).view(N, Len_q, self.n_heads, 3* self.n_points)
