...
from rfdetr.models.UMQS import UMQS
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, sa_nhead=8, ca_nhead=8, num_queries=300,
        ...
        super().__init__()
        self.encoder = None
        self.umqs = UMQS(num_queries=num_queries, keep_queries=100, query_dim=d_model)
    ...
    def forward(self, srcs, masks, pos_embeds, refpoint_embed, query_feat):
        ...
        if not self.two_stage:
            # Step 1: Expand query_feat and refpoint_embed to [B, N, C] and [B, N, 4]
            raw_query_feat = query_feat.unsqueeze(0).repeat(bs, 1, 1)  # [B, 300, C]
            refpoint_embed = refpoint_embed.unsqueeze(0).repeat(bs, 1, 1)

            if hasattr(self, 'umqs'):
                tgt, refpoint_embed = self.umqs(raw_query_feat, refpoint_embed)  # Apply UMQS
                self.group_detr = 1  # Prevent splitting in decoder
            else:
                tgt = raw_query_feat
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, sa_nhead, ca_nhead, dim_feedforward=2048, dropout=0.1,
                 ...
                 num_feature_levels=3, dec_n_points=4, 
                 skip_self_attn=False):
        super().__init__()
    ...
    def forward_post(self, tgt, memory,
        ...
        if self.training and (num_queries % self.group_detr == 0):
            ...
            # skip splitting if not divisible or UMQS reduced number of queries
            self.group_detr = 1
