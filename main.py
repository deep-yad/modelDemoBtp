...
from rfdetr.models import reconstruction_decoder
...
class UnlabeledDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, transform=None):
        ...
        # Default transform: Convert to tensor and normalize like COCO images
    ...
    Args:
        images (Tensor): Input images [B, C, H, W] in range [0, 1] or [0, 255]
        mean (float): Mean of the Gaussian noise
        std (float): Standard deviation of the Gaussian noise
    Returns:
        Tensor: Noisy images (clamped to valid range)
    """
...
class Model:
    ...
    def train(self, callbacks: DefaultDict[str, list], **kwargs):
        ...
        for key in callbacks.keys():
            if key not in currently_supported_callbacks:
                raise ValueError(
                    f"Callback {key} is not currently supported.\n"
        ...
        self.reconstruction_decoder = reconstruction_decoder.ConvNeXtReconstructionDecoder(**kwargs)
    ...
    # ==== DATASETS ====
        ...
        dataset_unlabeled = UnlabeledDataset("/home/deepak/rf-detr-develop-convnextV2-umqs-ssl/rfdetr/dataset_detr/unlabeled_data")
    ...
    # Samplers
        if args.distributed:
            ...
            sampler_unlabeled = DistributedSampler(dataset_unlabeled)
        else:
            sampler_train = torch.utils.data.RandomSampler(dataset_train)
            sampler_val = torch.utils.data.SequentialSampler(dataset_val)
            sampler_unlabeled = torch.utils.data.RandomSampler(dataset_unlabeled)

    # DataLoaders
        batch_sampler_train = torch.utils.data.BatchSampler(sampler_train, args.batch_size * args.grad_accum_steps, drop_last=True)
        data_loader_train = DataLoader(dataset_train, batch_sampler=batch_sampler_train,
                                   collate_fn=utils.collate_fn, num_workers=args.num_workers)
        data_loader_val = DataLoader(dataset_val, args.batch_size, sampler=sampler_val,
                                 drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
    # ==== SSL CHANGE: unlabeled loader ====
        data_loader_unlabeled = DataLoader(dataset_unlabeled, args.batch_size,
                                       sampler=sampler_unlabeled, drop_last=False,
                                       collate_fn=utils.collate_fn, num_workers=args.num_workers)
    ...
    # Dropout scheduling
        total_batch_size = args.batch_size * args.grad_accum_steps * utils.get_world_size()
        num_training_steps_per_epoch = (len(dataset_train) + total_batch_size - 1) // total_batch_size
        schedules = {}
        if args.dropout > 0:
            schedules['do'] = drop_scheduler(args.dropout, args.epochs, num_training_steps_per_epoch,
                                         args.cutoff_epoch, args.drop_mode, args.drop_schedule)
        if args.drop_path > 0:
            schedules['dp'] = drop_scheduler(args.drop_path, args.epochs, num_training_steps_per_epoch,
                                         args.cutoff_epoch, args.drop_mode, args.drop_schedule)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            model.train()
            criterion.train()
            if hasattr(self, 'reconstruction_decoder'):
                self.reconstruction_decoder.train()

        # ==== SUPERVISED TRAINING ====
            train_stats = train_one_epoch(
                model, criterion, lr_scheduler, data_loader_train, optimizer, device, epoch,
                args.batch_size * args.grad_accum_steps, args.clip_max_norm, ema_m=self.ema_m,
                schedules=schedules, num_training_steps_per_epoch=num_training_steps_per_epoch,
                vit_encoder_num_layers=args.vit_encoder_num_layers, args=args, callbacks=callbacks
            )

            # ==== SSL CHANGE: UNSUPERVISED TRAINING ====
            try:
            ...
            # Extract actual tensor from NestedTensor
            imgs = unlabeled_samples.tensors  # shape [B, C, H, W]
            noisy_imgs = add_gaussian_noise(imgs)
            # Re-wrap into NestedTensor if model expects it
            noisy_unlabeled = type(unlabeled_samples)(noisy_imgs, unlabeled_samples.mask)


            encoder_feats = model.backbone(noisy_unlabeled)  # backbone forward
            flat_feats = []
            for feat in encoder_feats:
                if isinstance(feat, torch.Tensor):
                    ...
                    for sub_feat in feat:
                        if isinstance(sub_feat, torch.Tensor):
                            flat_feats.append(sub_feat)
                        elif hasattr(sub_feat, "tensors"):  # Handle NestedTensor
                            flat_feats.append(sub_feat.tensors)
                        else:
                            raise ValueError(f"Unexpected sub-feature type: {type(sub_feat)}")
                elif hasattr(feat, "tensors"):  # Handle NestedTensor at top-level
                    flat_feats.append(feat.tensors)
                else:
                    raise ValueError(f"Unexpected feature type: {type(feat)}")

            # Debug: print shapes
            print("\n--- DEBUG: Flattened Encoder Features ---")
            for i, f in enumerate(flat_feats):
                print(f"Flat Feature[{i}]: shape = {f.shape}")
            print("--- END DEBUG ---\n")

            # Dynamically get channel sizes
            actual_in_channels = [f.shape[1] for f in flat_feats]
            
            # Create decoder dynamically
            reconstructed_imgs = self.reconstruction_decoder(flat_feats)
            # Ensure both inputs are tensors for loss
            if hasattr(unlabeled_samples, "tensors"):
                unlabeled_samples = unlabeled_samples.tensors
            if hasattr(reconstructed_imgs, "tensors"):
                reconstructed_imgs = reconstructed_imgs.tensors
            

            unlabeled_samples = F.interpolate(
                    unlabeled_samples, 
                    size=reconstructed_imgs.shape[2:],  # match H, W
                    mode='bilinear', 
                    align_corners=False
            )
            ssl_weight=1
            loss_unsupervised = F.l1_loss(reconstructed_imgs, unlabeled_samples)

            loss_unsupervised = ssl_weight * loss_unsupervised

            optimizer.zero_grad()
            loss_unsupervised.backward()
            optimizer.step()

            print(f"[SSL] Epoch {epoch} | Consistency Loss: {loss_unsupervised.item():.4f}")
         # ==== END CHANGE ====

        # ==== VALIDATION & CHECKPOINTS ====
            with torch.inference_mode():
                test_stats, coco_evaluator = evaluate(model, criterion, postprocessors, data_loader_val, base_ds, device, args=args)
...
def get_args_parser():
    ...
    parser.add_argument('--projector_scale', default='P3', type=str, nargs='+', choices=('P3', 'P4', 'P5', 'P6'))
...
def main(**kwargs):
    # Convert kwargs to argparse.Namespace for compatibility
    args = populate_args(**kwargs)
